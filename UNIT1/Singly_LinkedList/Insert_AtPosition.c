...
NODE Insert_atPos(NODE Head , int pos , int data)
{
    NODE newNode = create_node(data);
    if(Head == NULL)
    {
    ...
    while(temp != NULL && position < (pos-1))
    {
     ...
     if(temp == NULL)
    {
        printf("Out of bound and will be inserted at end\n");
        return(Insert_rear(Head,data));
    }
...
int main()
{
    ...
    node = Insert_atPos(node , 0 ,20);
    Display(node);

    node = Insert_atPos(node , 10 ,80);
    Display(node);
